...
const Navbar = () => {
  ...
  const activeStyle = {
    color: '#292929',
    boxShadow: 'inset 0 -7px 0 #8dd6a2',
  ...
  return (
    <nav className={`fixed w-screen top-0 left-0 z-[100] transition-all duration-700 ease-out ${isScrolled || open ? 'bg-off-white shadow-2xl ' : 'bg-transparent shadow-none'}`}>
      ...
      <ul className={`absolute lg:static -z-10 lg:z-auto text-gray-dark lg:text-gray-light bg-off-white lg:bg-transparent right-0 w-full lg:w-auto uppercase text-[1.16rem] lg:text-[1.16rem] tracking-widest font-medium leading-[0.938rem] pt-8 transition-all duration-500 ease-in lg:transition-none lg:pt-0 lg:flex lg:items-center lg:gap-10 opacity-0 ${isScrolled ? 'lg:opacity-100 lg:bg-transparent' : 'lg:opacity-0'} ${open ? 'translate-x-0 opacity-100 text-center shadow-2xl' : 'translate-x-full text-center lg:translate-x-0 '}`}>
